...
function new_event(mail: GoogleAppsScript.Gmail.GmailMessage) {
	...
	const new_err = (reason: string, id: string) => {
	...
	if (events === undefined) {
		throw new_err('failed to get calendar', 'no id');
	}


	// Calendarへの登録
	Template.calendar_ids.forEach((calendar_id) => {
		// 既存のイベントとの重複を確認
		events.list(calendar_id).items?.filter((event) => {
...
function parse_date(lines: string[]): [string, string] {
	let date_info = lines.find((line) => {
		return line.includes('開催日:');
	});

	const idx_of = (indicator: string) => {
		const idx = date_info?.indexOf(indicator);
		if (idx === undefined) {
			throw err_msg_template('calendar::parse_date::idx_of', `${indicator} is not found`, 'no id');
		}
		return idx;
	};
